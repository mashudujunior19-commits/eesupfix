...
def generate_hash(params: OzowRequest):
    ...
    hash_str = f"{payment['site_code']}{country_code}{currency_code}" \
...
def process_ozow_notification(params: OzowRequest):
    ...
    try:
        ...
        if amount != params.amount | trans_status != params.status:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail='Unexpected transaction format')

        allowed_statuses = ['Complete', 'Cancelled', 'Pending']

        if allowed_statuses.count(trans_status) > 0:
            count = supabase_client().schema('finances').table('online_payment').update(
                {'confirmed_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f+00'), 'status': trans_status}).eq('reference_id', params.transactionReference).eq('type', params.paymentType).execute()
            # return {
            #     'status_code': status.HTTP_200_OK,
            #     'detail': count}
        else:
            count = supabase_client().schema('finances').table('online_payment').update(
                {'confirmed_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f+00'), 'status': 'Pending'}).eq('reference_id', params.transactionReference).eq('type', params.paymentType).execute()
            return {
                'status_code': status.HTTP_200_OK,
                'detail': count}
    except:
